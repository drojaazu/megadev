...
| Register **names and layout**, corroborated by Sega's own sources | **DOC** | Found 2026-08-14 in the library share, independent of the manual scans. `Sega Developer Tools/32X/DEMO/CD_V2A/INC/SUBCPU.INC` is Sega's own Sub CPU equate file and uses **the same names megadev arrived at**: `GA_STAMPSIZE`, `GA_STAMPMAPBASE`, `GA_IMGBUFVSIZE`, `GA_IMGBUFSTART`, `GA_IMGBUFOFFSET`, `GA_TRACEVECTBASE`, `GA_SUBCODEBUFIMG`. It also confirms two transcriptions read off the fax: `CDCMODE_EDT_MSK EQU $8000` (EDT is word bit 15) and `MEMORYMODE_WP7_MSK EQU $8000` (WP7 is word bit 15, so write protect occupies the **high** byte — the KB-34 correction). `CD_V1A/SOURCE/LODER/SP_PRG.SYM` independently lists `_stamp_map_base FF805AH`, `_trace_vector FF8066H`, `_image_vdot_size FF8064H`. |
| Byte-wise addressing of the split registers | **DOC** | `WORK/sega_dev_stuff/STRIKE/NUGAME/SP.S` is production Sega-era game code and addresses these registers exactly as D17 models them: `BCLR.B #2,$FF8003` and `BSET.B #0,$FF8003` for memory mode, `BTST.B #7,$FF800E` and `BSET.B #7,$FF800F` for the two halves of the communication flags, `MOVE.B $FF8004` for CDC mode. Sega's own `SUBCPU.INC` meanwhile defines *word* masks for the same registers. Both views were in use, which supports D17 being a modelling choice rather than a claim about the hardware. |
...
| KB-41 | `lib/xform.h` | **Fixed 2026-08-14 — needs hardware confirmation.** `to_xform_delta()` produced a two's complement value, but the trace vector step is **sign and magnitude**: the manual's bit table (printed p.39) shows a separate `+/-` cell at bit 15 with the magnitude in bits 14-0. Every negative step was therefore wrong *and inverted* — `-0.5` encoded as `0xFC00`, which the hardware reads as **-15.5**, and a larger negative input produced a smaller negative step. Positive steps were unaffected, which is consistent with this never having been noticed: `examples/transforms` only ever sweeps small deltas around 1.0. The practical effect is that **any rotation is impossible**, since rotating past an axis needs negative components. Fixed, with `xform_delta_make()` for calculated values and `xform_delta_negate()` for reversal, and pinned by Tier 1.5 assertions. The sign-magnitude reading is `DOC` from the manual's bit table but has **not** been confirmed on hardware; if a transform behaves oddly at exactly ±0, this is the first thing to re-check. | ✅ |
