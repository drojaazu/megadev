...
| LIB-11 | S2 | **done** | KB-13 — the C `hextoa8/16/32` wrote no terminator while the assembly wrote `0xFF`. The 0xFF form is correct: the Boot ROM print routines require it and treat 0x00 as a newline (`main/bios.h` BIOS_PRINT). C now writes it too, and both sides share `STRING_TERMINATOR` from the new `lib/str_util.def.h` rather than hardcoding their own. |
...
| BR-4 | S2 | open | **`RESET_GA` does not match the documented forced-reset pattern.** *The Hardware* p.56 gives it as exactly four accesses — `MOVE.W #$FF00,$A12002` then `MOVE.B` of `$03`, `$02`, `$00` to `$A12001` — and warns that "a Forced Reset is not executed by command patterns other than those mentioned above", with "attention must be paid to address and size". `lib/main/gate_arr.macro.s` (taken from Wonder Library) writes `$03` **twice** with a `nop` between. Either the manual's "exact pattern" is not literal, or the macro is not triggering a forced reset at all and has been getting by on the side effects of the individual writes. **Not changed** — this is hardware behaviour and cannot be settled here. Test on hardware, or find the Wonder Library source, before touching it. |
...
| ARCH-4 | S3 | **done** | D14 — 99 register accessors converted to the lvalue form and 100 call-site dereferences removed. Enforced by INV-10. Verified byte-identical output. |
...
| DOC-17 | S3 | **done** | The CD fader (`$FF8034`) and every other register's access width and bit-operation rules are transcribed from *The Hardware* p.21 into `docs/gate_array.md`. Fader is word-access only with no bit operations. |
| DOC-18 | S3 | **done** | D15 — 0-based word index, matching `vdp_regNN`, with the address in each group title. 75 registers annotated across both CPU sides. |
| ARCH-14 | S4 | open | The large memory regions (`word_ram`, `word_ram_2m`, `word_ram_1m`, `prg_ram`, `prg_ram_1m`) are plain pointers because no size constants exist for them. `docs/megacd_dev.md` records the hardware sizes (Word RAM 2 Mbit, PRG RAM 4 Mbit, Work RAM 0.5 Mbit), so they could become sized array lvalues per INV-11 and gain a working `sizeof`. Adding the constants means asserting hardware facts, so check them against the Hardware Manual first. |
...
| ID | Sev | Status | Item |
|---|---|---|---|
| HW-7 | S2 | **done** | **KB-34** — `GA_LED_R`/`GA_LED_G`, `GA_MEMMODE_WP` and Main-side `GA_CDC_DEST` were byte-relative, so their derived masks named the wrong bits. Confirmed against Hardware Manual pp. 57–58 and made register-relative; `FIELD_BYTE`/`FIELD_BPOS` added for bit opcodes (SPEC §9 D16, INV-12). |
| HW-8 | S2 | **done** | D17 applied to the Main side: `$A12000` → `GA_REG_INT2`/`GA_REG_SUBCPU`, `$A12002` → `GA_REG_WP`/`GA_REG_MEMMODE`, `$A12004` → a read-only byte register. Resolved the `GA_RAISE_INT2` / `GA_SUB_RESET` position collision. `RESET_GA`'s word write to `$A12002` is preserved deliberately; see BR-4. |
| HW-9 | S3 | **done** | `GA_REG_COMFLAGS` split into `GA_REG_COMFLAGS_MAIN` / `GA_REG_COMFLAGS_SUB` on both sides, with the read/write direction reversed between them. Also removed the word accessor: the manual (p.28) warns that a byte or word read of the pair may return stale data if the other CPU writes concurrently, while a single-bit test cannot — so word access was a hazard the type system was inviting. |
| HW-10 | S1 | **done** | Trace vector steps are **two's complement**, not sign-and-magnitude. Settled by tracing Sonic CD (Japan) through a recorded input playthrough of the special stage; see KB-41 and `megadev-testing/trace/sonic-cd-special-stage.log`, kept outside the repo per SPEC.md D19. The argument is geometric and does not depend on emulator accuracy: only two's complement keeps the step vector's direction constant across rows while its magnitude scales with distance. |
| DOC-19 | S3 | **done** | Every gate array register on both CPU sides is transcribed from *The Hardware*: Sub `$FF8000`–`$FF81FE`, Main `$A12000`–`$A1202E`. Two registers are partly unresolved because the scan is destroyed and the clean SPHERE set does not cover those pages — `$FF8034` (`EFDT`/`DEF` positions) and `$FF8068` (`STA`/`SAOR` positions); both are marked ASSUMED in SPEC §7 and both are BIOS-only registers. Remaining manual pages 40–54 are rotation/scaling *explanatory* material (stamp organisation, mapping examples, image buffer concepts), not register definitions — see DOC-23. |
| DOC-23 | S3 | **done** | Manual pp. 40–54 read and folded into `docs/rotation_scaling.md` as §8 (laying out Word RAM: the shared 256 KB budget, stamp 0 reserved as *no character* but usable as ordinary RAM, map cost per mode, and the units each register counts in) and §9 (the offset overspill trap — `Vcell` must exceed the actual cell size unless `Hdot` ≤ 8 — and the unwritten `Vcell × Hcell` vs `Vdot × Hdot` region). Four failure modes added. Two corrections fell out of the reading: priority applies to **graphics-operation** writes in 2M mode and to CPU writes only in 1M mode (p.54), which the guide had backwards; and the withdrawn KB-41 sign-and-magnitude claim still survived in `docs/rotation_scaling.md` ×2 and a `@warning` in `lib/sub/gate_arr.def.h`. **The premise was partly stale** — `docs/rotation_scaling.md` already existed from commit `27d1483`, so this was an extension, not a new guide. Note "special cases (p.45)" was wrong: printed page 45 is blank (SPEC §7). Not transcribed: the HFLIP/ROTATE composition order on p.42, too degraded to read safely — see DOC-24. |
| DOC-24 | **S2** | **done** | **`STAMP_ROTATE_90` and `STAMP_ROTATE_180` were swapped — fixed, see KB-42.** Settled 2026-08-14 without needing a hardware test: the maintainer read the p.42 glyph for `RT0=0, RT1=1` as P turned counter-clockwise, i.e. 90°, which extracts from the bits 14–13 field as `0b10` where the SDK had `0b01`. The bit-position transcription was verified correct at 600 dpi, so the fix is the constants, not the field. Rotation is counter-clockwise and is applied before `HFLIP`. Original analysis follows. **`STAMP_ROTATE_90` and `STAMP_ROTATE_180` may be swapped.** Resolved on 2026-08-14 by re-rendering manual p.42 at 600 dpi: the composition **order** is settled — rotation first, then `HFLIP`, because each cell of the combined row is the exact horizontal mirror of the rotate-only cell (recorded in `docs/rotation_scaling.md` §2.2). The **angle mapping is not**. Bit positions are agreed and megadev transcribes them faithfully (15 `HFLP`, 14 `RT1`, 13 `RT0`), so the field value is `RT1×2 + RT0` and the SDK assigns `0b01`→90°, `0b10`→180°. But the figure's `RT0=1, RT1=0` cell reads as P through **180°**, which under that bit order is field value `0b01` — i.e. 90° and 180° swapped. Either the glyph reading is wrong or the constants are. **Not changed**, because inferring a bit encoding from a degraded figure is exactly how KB-41 went wrong. **Settle empirically:** render one asymmetric stamp under all four `ROTATE` values and photograph the result; the 0° and 270° cases agree under both readings, so only 90° vs 180° needs distinguishing. Cheap now that `mode7` builds. |
| DOC-21 | S3 | **done** | `GA_REG_CDDCOMM0-9` renamed to `GA_REG_CDDSTAT0-4` (`$FF8038`–`$FF8040`, drive → Sub CPU) and `GA_REG_CDDCMD0-4` (`$FF8042`–`$FF804A`, Sub CPU → drive), mirroring the `COMSTAT` / `COMCMD` pair used for Main↔Sub communication. Same direction convention: the command goes out from the controlling CPU and the status comes back. No call sites existed. |
| DOC-22 | S3 | open | **The CDC device-destination values are named differently on each side.** Sub: `GA_CDCMODE_DD_{MAINREAD,SUBREAD,PCMDMA,PRGDMA,WRAMDMA}`. Main: `GA_CDC_DEST_{MAIN,SUB,PCM,PRG,WORD}`. Same five hardware values, two spellings — and the Main side was missing `PRG` entirely until 2026-08-14. Worth unifying, but it is a naming call and sits inside OD-1 (the Main/Sub namespace question), so it should be settled together with that. |
| DOC-20 | S2 | open | **Explain the split-register model in the markdown docs, and say where to go to exceed it.** `docs/gate_array.md` §"Some registers are 16 bit, some are a pair of byte registers" is a first pass written alongside D17, but it argues the decision from the SDK's side only. The documentation pass should (a) state plainly that the split is a **Megadev modelling choice, not a hardware boundary** — the hardware register really is 16 bits wide and the two halves really are adjacent; (b) explain *why* (the halves carry unrelated concerns, so a word write to set one silently clobbers the other — the Program RAM write protect is the concrete case); and (c) tell an advanced reader who wants to bypass the model that the per-register **access width and bit-operation rules** are what govern a direct word access, that `docs/gate_array.md` §"Access width and bit operations, per register" already transcribes them from *The Hardware* p.21, and that the official manual is the place to go for anything beyond that table. Note the standing rule from D15's discussion: citations to the official documentation belong in the **markdown**, not in the Doxygen commentary — the generated reference is meant to stand alone for the average developer, with the manual reserved for maintainers and for people deliberately going deeper. Applies to `docs/gate_array.md` primarily, and to whatever the manual says about register conventions. |
| DOC-26 | S3 | **done** | **`$FF8034` field positions recovered from the Software Development Manual (printed p.16).** They had been ASSUMED and recorded as unrecoverable, because *The Hardware* p.31 is destroyed in the fax and missing from the SPHERE set. The second manual's bit table is fully legible: `EFDT`=15, `FD00-10`=4–14, `DEF1`=3, `DEF0`=2, `SSF`=1, bit 0 unused. `GA_CDFADER_EFDT`, `GA_CDFADER_DEF` (with the four de-emphasis values) and `GA_CDFADER_SSF` added; `SSF` was previously absent entirely. Tier 1.5 assertions pin all four and check they tile the register. See SPEC §7. |
| DOC-25 | S2 | **done** | **Gate array register block cross-checked against the Software Development Manual, 2026-08-14.** Swept: `$FF8000`, `$FF8002`, `$FF8004`, `$FF8006`, `$FF8008`, `$FF800C`, `$FF800E`, `$FF8030`, `$FF8032`, `$FF8034`, `$FF8036`, `$FF8038`-`$FF804A`, `$FF804C`, `$FF8058`, `$FF805C`, `$FF805E`, `$FF8064`, `$FF8066`, `$FF8068`. **Two defects fixed:** KB-43 (`ga_reg_cdchostdata` writable on the Sub side though `$FF8008` has no writable bits — KB-39's Main-side fix had not carried across) and the `$FF8068` accessor, likewise read only. **Two gaps closed:** `$FF8034` (DOC-26) and `$FF8068` field positions. **One field added:** `GA_STOPWATCH_SW` (12 bits), which had no mask so reads were not masked against the unused top nibble. **Corroborated with no change:** all `$FF8000`/`$FF8002` positions; the priority encoding (`PM1,PM0` = 00 OFF / 01 Write down / 10 Write up / 11 prohibited, matching `GA_PRIORITY_*` exactly); the D17/HW-9 comm flag split; the D18 CDD direction split and nibble layout; interrupt mask, timer, font colour, VCS, VW and both base-address `>>2` encodings. **Not swept** (no megadev field claims at risk): `$FF800A`, `$FF804E`, `$FF8050`-`$FF8056`, `$FF8060`, `$FF8062`. Five questions raised and deliberately not acted on — see DOC-27. |
| DOC-27 | S3 | open | **Five discrepancies between the two Sega manuals, recorded but not acted on.** Each needs *The Hardware* or hardware to settle, and none is urgent. **(1)** `$FF8000`: the Software Manual's RD row marks `Ver3-Ver0` as reading 0, which would make megadev's `GA_VERSION` unreadable. **(2)** `$FF8002`: the WR row marks `DMNA` not writable from the Sub CPU (only `RET` is), but megadev's Sub-side `@param DMNA` describes writing it — misleading wording at minimum. **(3)** `$FF8004`: the Software Manual marks bits 13-11 unused where megadev has `GA_CDCMODE_UBR` at 13; the Software Manual is Ver 0.10 and seven months earlier, so *The Hardware* p.26 probably added it. **(4)** `$FF8066`: the Software Manual's RD row is `0/1`, i.e. readable, but megadev's Doxygen warns "Write only"; the accessor is already read/write so nothing breaks. **(5)** `ga_reg_cddstat0-4` are typed writable though they carry status from the drive (KB-38 shape); the Software Manual's CDD table is one register per row with no RD/WR row, so it cannot settle it. |
