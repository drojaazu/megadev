...
| DOC-24 | S4 | open | **The order in which `HFLIP` and `ROTATE` are applied to a stamp is unrecorded.** The manual tabulates all eight orientations (p.42) but the figure is a degraded fax and the glyphs cannot be read reliably in any available scan, including the Kodak one. `docs/rotation_scaling.md` §2.2 says so explicitly rather than guessing. Settle empirically: render one asymmetric stamp under all four `ROTATE` values with `HFLIP` set and clear, and compare. Cheap to do once the `mode7` example is running. |
