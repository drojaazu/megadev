...
| HW-8 | S2 | open | **D17 follow-through, Main side.** `$A12000` (INT2/BUSREQ vs reset) and `$A12002` (WP vs bank/mode) are the Main-CPU equivalents of the registers split on the Sub side, and `GA_MEMMODE_WP`/`GA_CDC_DEST` still carry word-relative positions there. `RESET_GA` in `lib/main/gate_arr.macro.s:63` does `move.w #0xFF00, GA_REG_MEMMODE`, a deliberate both-halves write taken from Wonder Library — preserve it explicitly rather than silently turning it into two byte writes. |
