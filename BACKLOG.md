...
| DOC-25 | S2 | **in-progress** | **Cross-check the whole gate array register block against the Software Development Manual.** Both known field-position gaps are now closed: `$FF8034` (DOC-26) and `$FF8068` (`SAOR` bit 7, `STA6-1` bits 6-1, bit 0 unused, whole register **read only** — accessor retyped `ga_reg_ro`, fields and Tier 1.5 assertions added). **Corroborated, no change needed:** the CDD communication nibble layout (even index at bits 11-8, odd at bits 3-0; `$FF8038`-`$FF8040` receive status, `$FF8042`+ transfer command — confirms D18); `$FF8036` `DRS`/`HOCK`/`D/M` semantics; the trace vector fractional bit counts; `VCS` stored minus one. **Still unswept:** `$FF8000` reset/LED, `$FF8002` memory mode, `$FF8004`/`$FF8006` CDC, `$FF8008` host data, `$FF800A` DMA address, `$FF800C` stopwatch, `$FF800E` comm flags, `$FF8030` timer, `$FF8032` interrupt mask, `$FF804C`/`$FF804E` font, and the image buffer group `$FF805C`-`$FF8066`. **Specific open question:** `ga_reg_cddstat0-4` are typed writable (`ga_reg`) though they carry status *from* the drive — the same shape as KB-38. The Software Manual's CDD table is one register per row with no RD/WR row, so it does not settle it; check *The Hardware* p.33 or the BIOS manual. |
