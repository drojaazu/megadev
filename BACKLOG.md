...
| DOC-25 | S2 | open | **Cross-check the whole gate array register block against the Software Development Manual.** The register block is documented in *two* Sega manuals and they disagree; `RPT` is a proven case where the Software Manual is wrong (SPEC §7, corrections email). DOC-19 transcribed only from *The Hardware*, so every register it settled has an unread second opinion. `Mega-CD Software Development.pdf` has an OCR text layer, so this is a cheap sweep rather than a page-by-page read. **Priority targets:** `$FF8034` (`EFDT`/`DEF` positions, still ASSUMED and unrecoverable from *The Hardware*) and `$FF8068` (`STA`/`SAOR`, same). Record any disagreement as a live question, not a correction. |
