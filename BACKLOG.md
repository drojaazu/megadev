...
| DOC-19 | S3 | open | Sub-side registers still untranscribed: font `$FF804C`–`$FF8056` (p. 34), rotation/scaling `$FF8058`–`$FF8066` (pp. 35–54, the largest block by far), sub-code `$FF8068`+ (p. 55). Done: `$FF8000`–`$FF804A`. Page map and the better SPHERE scan set are in SPEC §7. |
| DOC-21 | S3 | open | **`GA_REG_CDDCOMM0-9` are named generically but are two different things.** Manual p.33: `$FF8038`–`$FF8040` carry Receiving Status 0–9 (drive → Sub CPU) and `$FF8042`–`$FF804A` carry Transmission Command 0–9 (Sub CPU → drive). The current names say only "CDD comm N". Renaming to e.g. `GA_REG_CDDSTAT0-4` / `GA_REG_CDDCMD0-4` would say which direction each is, but it is an API change on ten registers and the naming call is Damian R's — the block is documented accurately either way, and the Sub BIOS is the supported route to it. |
