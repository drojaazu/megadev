...
| DOC-23 | S3 | open | Manual pp. 40–54 explain the rotation/scaling system rather than defining registers: stamp data organisation (p.42), mapping examples (p.43), special cases (p.45), image buffer concept and map (pp.52–53), offset and priority (p.54). Worth turning into a `docs/` guide, since the register documentation now assumes this background but nothing in the repo supplies it. Not register transcription, so it is not part of DOC-19. |
