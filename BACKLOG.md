...
| MAKE-15 | S3 | open | Module links pass `-z muldefs`, which is **necessary** — modules sharing a layout file duplicate `MODULE_ROM_LENGTH` etc. against the resident module they import with `-R`; removing it fails `new_project`. But it is blunt: it suppresses *all* duplicate-symbol errors, including real ones. Consider scoping it, e.g. by not exporting layout symbols from the ELF used with `-R`. |
