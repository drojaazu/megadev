...
| DOC-16 | S3 | open | Megadev provides no `_HI`/`_LO` aliases for the byte halves of byte-accessible gate array registers; current practice is `GA_REG_MEMMODE + 1`. Adding them requires knowing which registers permit byte access, which is in *Mega-CD Hardware Manual - The Hardware* (SPEC.md §7). `docs/gate_array.md` records the gap rather than documenting a convention that does not exist. |
