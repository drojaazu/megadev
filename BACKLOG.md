...
| HW-10 | S1 | open | **Confirm on hardware that trace vector steps are sign and magnitude** (KB-41). The manual's bit table shows a separate `+/-` bit, and `lib/xform.h` now encodes that way, but it has only been read off a fax scan. A five minute test settles it: render with a step of `-1.0` encoded both ways and see which produces a mirrored image rather than a wildly scaled one. Everything about rotation depends on this being right. |
