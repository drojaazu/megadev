...
| DOC-25 | S2 | open | **Cross-check the whole gate array register block against the Software Development Manual.** First result is DOC-26, which closed a claim believed unrecoverable — evidence the sweep is worth doing. The register block is documented in *two* Sega manuals and they disagree; `RPT` is a proven case where the Software Manual is wrong (SPEC §7, corrections email). DOC-19 transcribed only from *The Hardware*, so every register it settled has an unread second opinion. `Mega-CD Software Development.pdf` has an OCR text layer, so this is a cheap sweep rather than a page-by-page read. **Priority target:** `$FF8068` (`STA`/`SAOR`), still ASSUMED. Record any disagreement as a live question, not a correction. |
