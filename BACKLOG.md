...
| DOC-18 | S3 | **done** | Resolved by the source: **Sega does not number the gate array registers at all** — the manual identifies them by address (`$FF8002 : Memory mode`). Neither "GA Reg 01" nor "Reg. #2" matches. Documentation groups should key on the address or the functional block, not an invented index. |
