...
| HW-9 | S3 | open | **D17 follow-through, `GA_REG_COMFLAGS` (`$FF800E` / `$A1200E`).** Its halves are already distinct in the C accessors (`ga_reg_comflags_main` / `_sub`) but the address macro is still one word. Splitting it finishes the pattern; low priority because the accessors already prevent the mistake. |
| DOC-19 | S3 | open | Sub-side registers still untranscribed from the Hardware Manual: `$FF800C`–`$FF802E` (pp. 28–29), `$FF8030`/`$FF8032` (p. 30), CDD `$FF8034`–`$FF804A` (pp. 31–33), font `$FF804C`–`$FF8056` (p. 34), rotation/scaling `$FF8058`–`$FF8066` (pp. 35–54), sub-code `$FF8068`+ (p. 55). Page map in SPEC §7. |
