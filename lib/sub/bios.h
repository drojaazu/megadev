...
{
  register u16 _CDB_DRIVE_OPEN asm("d0") = CDB_DRIVE_OPEN;

  asm volatile(
    "\
		jsr %p1"
    : "+d"(_CDB_DRIVE_OPEN)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_DRIVE_OPEN)
    : "cc", "a0", "a1");
};
 ...
 *
 * @details
 * This is for use with the CDB_DRIVE_INIT call. Unless you have done some CD
...
/**
 * @fn bios_drive_init(struct DriveInitParams const *)
 * @brief Reads the TOC from the disc
 * @sa CDB_DRIVE_INIT
 * @ingroup sub_bios_misc
 *
 * @details
 * Pauses for 2 seconds after reading the TOC. Waits for a CDB_DRIVE_OPEN
 * request if there is no disk in the drive. Also closes the drive door for
 * Model 1 hardware
 *
 * @note
 * Consider using bios_drive_init() instead, which uses the standard settings
 */
static inline void
bios_drive_init(struct DriveInitParams const * drive_init_params)
{

  register u16 _CDB_DRIVE_INIT asm("d0") = CDB_DRIVE_INIT;
  register u32 _drive_init_params asm("a0") = (u32) drive_init_params;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_DRIVE_INIT), "+a"(_drive_init_params)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_DRIVE_INIT), "a"(_drive_init_params)
    : "cc", "a1");
};
...
{
  register u16 D0 asm("d0") = CDB_CHECK_STATUS;
...
static inline void bios_wait_for_call_complete()
{
  asm(
    ...
    1:move.w #%p1, d0 \n\
		...
		"
    :
    : "i"(CDBIOS_VECTOR), "i"(CDB_CHECK_STATUS)
    : "cc", "d0");
}
...
{
  register u16 D0 asm("d0") = CDB_GET_STATUS;
...
{
  register u16 _CDB_AUDIO_PLAY asm("d0") = CDB_AUDIO_PLAY;
  register u32 _track_number asm("a0") = (u32) track_number;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_AUDIO_PLAY), "+a"(_track_number)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_PLAY), "a"(_track_number)
    : "cc", "a1");
};
 ...
 * @sa CDB_AUDIO_PLAY_ONCE
 * @ingroup sub_bios_cdda
 */
static inline void bios_audio_play_once(u16 const * track_number)
{
  register u16 _CDB_AUDIO_PLAY_ONCE asm("d0") = CDB_AUDIO_PLAY_ONCE;
  register u32 _track_number asm("a0") = (u32) track_number;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_AUDIO_PLAY_ONCE), "+a"(_track_number)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_PLAY_ONCE), "a"(_track_number)
    : "cc", "a1");
};
 ...
 * @sa CDB_AUDIO_PLAY_REPEAT
 * @ingroup sub_bios_cdda
 */
static inline void bios_audio_play_repeat(u16 const * track_number)
{
  register u16 _CDB_AUDIO_PLAY_REPEAT asm("d0") = CDB_AUDIO_PLAY_REPEAT;
  register u32 _track_number asm("a0") = (u32) track_number;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_AUDIO_PLAY_REPEAT), "+a"(_track_number)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_PLAY_REPEAT), "a"(_track_number)
    : "cc", "a1");
};
 ...
 * @sa CDB_AUDIO_PLAY_FROM
 * @ingroup sub_bios_cdda
 */
static inline void bios_audio_play_from(u32 const * timecode)
{
  register u16 _CDB_AUDIO_PLAY_FROM asm("d0") = CDB_AUDIO_PLAY_FROM;
  register u32 _timecode asm("a0") = (u32) timecode;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_AUDIO_PLAY_FROM), "+a"(_timecode)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_PLAY_FROM), "a"(_timecode)
    : "cc", "a1");
};
...
{
  register u16 _CDB_AUDIO_SEEK asm("d0") = CDB_AUDIO_SEEK;
  register u32 _track_number asm("a0") = (u32) track_number;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_AUDIO_SEEK), "+a"(_track_number)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_SEEK), "a"(_track_number)
    : "cc", "a1");
};
 ...
 * @sa CDB_AUDIO_SEEK_ONCE
 * @ingroup sub_bios_cdda
 */
static inline void bios_audio_seek_once(u16 const * track_number)
{
  register u16 _CDB_AUDIO_SEEK_ONCE asm("d0") = CDB_AUDIO_SEEK_ONCE;
  register u32 _track_number asm("a0") = (u32) track_number;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_AUDIO_SEEK_ONCE), "+a"(_track_number)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_SEEK_ONCE), "a"(_track_number)
    : "cc", "a1");
};
 ...
 * @sa CDB_AUDIO_SEEK_FROM
 * @ingroup sub_bios_cdda
 */
static inline void bios_audio_seek_from(u32 const * timecode)
{
  register u16 _CDB_AUDIO_SEEK_FROM asm("d0") = CDB_AUDIO_SEEK_FROM;
  register u32 _timecode asm("a0") = (u32) timecode;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_AUDIO_SEEK_FROM), "+a"(_timecode)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_SEEK_FROM), "a"(_timecode)
    : "cc", "a1");
};
...
{
  register u16 _CDB_AUDIO_STOP asm("d0") = CDB_AUDIO_STOP;

  asm volatile(
    "\
		jsr %p1"
    : "+d"(_CDB_AUDIO_STOP)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_STOP)
    : "cc", "a0", "a1");
}
...
{
  register u16 _CDB_AUDIO_PAUSE_ON asm("d0") = CDB_AUDIO_PAUSE_ON;

  asm volatile(
    "\
		jsr %p1"
    : "+d"(_CDB_AUDIO_PAUSE_ON)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_PAUSE_ON)
    : "cc", "a0", "a1");
}
...
{
  register u16 D0 asm("d0") = CDB_AUDIO_PAUSE_OFF;
 ...
 * @sa CDB_AUDIO_SCAN_FF
 * @ingroup sub_bios_cdda
 */
static inline void bios_audio_scan_ff()
{
  register u16 _CDB_AUDIO_SCAN_FF asm("d0") = CDB_AUDIO_SCAN_FF;

  asm volatile(
    "\
		jsr %p1"
    : "+d"(_CDB_AUDIO_SCAN_FF)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_SCAN_FF)
    : "cc", "a0", "a1");
};
 ...
 * @sa CDB_AUDIO_SCAN_FR
 * @ingroup sub_bios_cdda
 */
static inline void bios_audio_scan_fr()
{
  register u16 _CDB_AUDIO_SCAN_FR asm("d0") = CDB_AUDIO_SCAN_FR;

  asm volatile(
    "\
		jsr %p1"
    : "+d"(_CDB_AUDIO_SCAN_FR)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_SCAN_FR)
    : "cc", "a0", "a1");
};
...
{
  register u16 _CDB_AUDIO_SCAN_OFF asm("d0") = CDB_AUDIO_SCAN_OFF;

  asm volatile(
    "\
		jsr %p1"
    : "+d"(_CDB_AUDIO_SCAN_OFF)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_AUDIO_SCAN_OFF)
    : "cc", "a0", "a1");
};
...
{
  register u16 _CDB_ROM_READ asm("d0") = CDB_ROM_READ;
  register u32 _sector_number asm("a0") = (u32) sector_number;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_ROM_READ), "+a"(_sector_number)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_ROM_READ), "a"(_sector_number)
    : "cc", "a1");
};
...
{
  register u16 _CDB_ROM_READ_COUNT asm("d0") = CDB_ROM_READ_COUNT;
  register u32 _param asm("a0") = (u32) param;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_ROM_READ_COUNT), "+a"(_param)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_ROM_READ_COUNT), "a"(_param)
    : "cc", "a1");
};
...
{
  register u16 _CDB_ROM_READ_RANGE asm("d0") = CDB_ROM_READ_RANGE;
  register u32 _param asm("a0") = (u32) param;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_ROM_READ_RANGE), "+a"(_param)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_ROM_READ_RANGE), "a"(_param)
    : "cc", "a1");
};
...
{
  register u16 _CDB_ROM_PAUSE_ON asm("d0") = CDB_ROM_PAUSE_ON;

  asm volatile(
    "\
		jsr %p1"
    : "+d"(_CDB_ROM_PAUSE_ON)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_ROM_PAUSE_ON)
    : "cc", "a0", "a1");
};
...
{
  register u16 _CDB_ROM_PAUSE_OFF asm("d0") = CDB_ROM_PAUSE_OFF;

  asm volatile(
    "\
		jsr %p1"
    : "+d"(_CDB_ROM_PAUSE_OFF)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_ROM_PAUSE_OFF)
    : "cc", "a0", "a1");
};
...
{
  register u16 _CDB_ROM_SEEK asm("d0") = CDB_ROM_SEEK;
  register u32 _sector_number asm("a0") = (u32) sector_number;

  asm volatile(
    "\
		jsr %p2"
    : "+d"(_CDB_ROM_SEEK), "+a"(_sector_number)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_ROM_SEEK), "a"(_sector_number)
    : "cc", "a1");
};
...
{
  register u16       _CDB_FADER_SET asm("d0") = CDB_FADER_SET;
  ...
  asm volatile(
    "\
		jsr %p1"
    : "+d"(_CDB_FADER_SET)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_FADER_SET), "d"(_volume)
    : "cc");
};
...
{
  register u16 _CDB_FADER_CHANGE asm("d0") = CDB_FADER_CHANGE;
  ...
  asm volatile(
    "\
		jsr %p1"
    : "+d"(_CDB_FADER_CHANGE)
    : "i"(CDBIOS_VECTOR), "d"(_CDB_FADER_CHANGE), "d"(_volume)
    : "cc");
};
...
{
  register u16 D0 asm("d0") = CDB_CDC_START;
...
{
  register u16 D0 asm("d0") = CDB_CDC_STOP;
...
{
  register u16 D0 asm("d0") = CDB_CDC_STATUS;
...
{
  register u16  D0 asm("d0") = CDB_CDC_TRANSFER;
...
jsr %p3"
    ...
    : "i"(CDBIOS_VECTOR), "d"(D0), "a"(a0_sector_dest), "a"(a1_header_dest)
    : "cc", "d1");
};
...
{
  register u16 D0 asm("d0") = CDB_CDC_ACK;
