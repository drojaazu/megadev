...
#define print_xy(x,y) (VDPPTR(NMT_POS_PLANE(x,y,BOOT_PLANE_WIDTH,BOOT_PLANEA_ADDR)) | VRAM_W)
...
void bram_brmdir_test() {
	...
	*GA_COMCMD0 = 8;
	...
	char * filename;
	while(1) {
		if(*(u8*)(MAIN_2M_BASE + (file_idx * 16)) == 0) break;
		filename = (char*)(MAIN_2M_BASE + (file_idx * 16));
		filename[11] = 0xff;
		boot_print(filename, print_xy(1, curr_y));
		printval_u16_c(*(u16*)(MAIN_2M_BASE + (file_idx * 16) + 14), val_buffer);
	...
	*VDP_DATA_16 = 0x0EEE;
	...
	do {
		...
		do {
			...
			{
				*VDP_CTRL_32 = (VDPPTR(NMT_POS_PLANE(1,clearloop,BOOT_PLANE_WIDTH,BOOT_PLANEA_ADDR)) | VRAM_W);
				*VDP_DATA_16 = 0;
			}
			*VDP_CTRL_32 = (VDPPTR(NMT_POS_PLANE(1,(menupos + 3),BOOT_PLANE_WIDTH,BOOT_PLANEA_ADDR)) | VRAM_W);
			*VDP_DATA_16 = '>';
