...
#define print_xy(x, y) (VDPPTR(NMT_POS_PLANE(x, y, BOOT_PLANEA_ADDR)) | VRAM_W)
...
void bram_brmdir_test() {
  ...
  *GA_COMCMD0 = 8;
  ...
  char * filename;
  while (1) {
    if (*(u8 *)(MAIN_2M_BASE + (file_idx * 16)) == 0)
      break;
  ...
  do {
    ...
    do {
      ...
      for (int clearloop = 3; clearloop < 10; ++clearloop) {
        *VDP_CTRL_32 =
            (VDPPTR(NMT_POS_PLANE(1, clearloop, BOOT_PLANEA_ADDR)) | VRAM_W);
        *VDP_DATA_16 = 0;
      }
      *VDP_CTRL_32 =
          (VDPPTR(NMT_POS_PLANE(1, (menupos + 3), BOOT_PLANEA_ADDR)) | VRAM_W);
      *VDP_DATA_16 = '>';
