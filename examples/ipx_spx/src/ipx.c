...
void init_particle(u8 particle_idx) {
	...
	SPRITE_LIST[particle_idx].size = settings.main_tile_size;
}
...
void init_particles(u16 main_tile, u16 end_tile, u8 main_tile_size, u8 end_tile_size,
	...
	settings.main_tile_size = main_tile_size;
	settings.end_tile_size = end_tile_size;
...
void process_particles() {
	for(u8 iter = 0; iter < 16; ++iter) {
		...
		if(particles[iter].status == Ending) {
			...
			SPRITE_LIST[iter].size = settings.end_tile_size;
