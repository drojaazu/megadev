...
const VDPCMD vdpptr_scroll = VDPPTR((_BIOS_HSCROL_ADDR)) | VDPOP_VRAM_W;
...
__attribute__((noreturn)) void main()
{
	...
	VDP_CTRL_32 = VDPPTR(VRAM_AT(1)) | VDPOP_VRAM_W;
	...
	VDP_CTRL_32 = VDPPTR(VRAM_AT(free_tile)) | VDPOP_VRAM_W;
	bios_gfx_decomp(res_cybercity_farbg_cmp_nem);

	free_tile += ((*(u16 *) res_cybercity_farbg_cmp_nem) & 0x7fff);

	bios_dma_xfer_wrdram(VDPPTR(VRAM_AT(free_tile)) | VDPOP_VRAM_W, res_ship_chr, 1920 >> 1);

	bios_load_map(VDPPTR(NMT_POS_PLANE(0, 2, _BIOS_PLANEA_ADDR)) | VDPOP_VRAM_W,
		res_cybercity_bldg_map[0] - 1,
		res_cybercity_bldg_map[1] - 1,
		res_cybercity_bldg_map + 2);
	bios_load_map(VDPPTR(_BIOS_PLANEB_ADDR) | VDPOP_VRAM_W,
