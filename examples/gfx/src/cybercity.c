...
/*
	We mark this is as noreturn since this is an infinite loop that will never
	escape. Hopefully this will hint some optimizations to GCC. Unfortunately,
	it looks like GCC will still push the registers on entry, taking up valuable
	stack space. I have not found a way around this (the naked attribute is not
	available on m68k arch). Any suggestions to prevent this would be appreciated!
*/
__attribute__ ((noreturn)) void main()  {
  ...
  spr_funcs[1] = null_func;
	...
	*VDP_CTRL_32 = VDPPTR(AT_TILE(1)) | VRAM_W;
	...
	// first word of Nemesis compression is the tile count, with the msb determining XOR mode
	// so we can use this (with the msb cleared) to get the next free tile
	// +1 to account for the blank 0 tile
	...
	*VDP_CTRL_32 = VDPPTR(AT_TILE(free_tile)) | VRAM_W;
	...
	boot_dma_xfer_wordram(VDPPTR(AT_TILE(free_tile)) | VRAM_W, res_ship_chr, 1920 >> 1);

	boot_load_map(VDPPTR(NMT_POS_PLANE(0,2,*PLANE_WIDTH,BOOT_PLANEA_ADDR)) | VRAM_W, res_cybercity_bldg_map[0] - 1, res_cybercity_bldg_map[1] - 1, res_cybercity_bldg_map + 2);
	boot_load_map(VDPPTR(BOOT_PLANEB_ADDR) | VRAM_W, res_cybercity_farbg_map[0] - 1, res_cybercity_farbg_map[1] - 1, res_cybercity_farbg_map + 2);
