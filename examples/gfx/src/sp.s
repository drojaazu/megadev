...
1:BIOSCALL #BIOS_STAT
  ...
  CLEAR_COMM_REGS
	...
	andi.w	#~(GA_CEDE_WDRAM2M | GA_WDRAM2M_MODE), GA_REG_MEMMODE
...
GLABEL sp_main
  ...
  /*
	...
	*/
command_loop:
  move.w	GA_REG_COMM_CMD0, d0
	beq			command_loop
	cmp.w		GA_REG_COMM_CMD0, d0
	bne			command_loop
	#move.w	d0, d1
	move.w  GA_REG_COMM_CMD1, d1
	add.w		d0, d0
...
command_complete_sync:
	move.w	GA_REG_COMM_CMD0, GA_REG_COMM_STAT0
1:move.w	GA_REG_COMM_CMD0, d0
	bne			1b
	move.w	GA_REG_COMM_CMD0, d0
	bne			1b
	move.w	#0, GA_REG_COMM_STAT0
	rts
